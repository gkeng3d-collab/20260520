...
## โปรแกรมหน้าเว็บ (ไม่ต้องติดตั้งอะไร)

เปิดไฟล์ **`app.html`** ในเบราว์เซอร์ (ดับเบิลคลิกได้เลย) — วาง/อัปโหลดไฟล์ราคา
หรือกดดึงจาก Yahoo แล้วโปรแกรมจะคำนวณสถิติ, correlation heatmap,
efficient frontier แบบเลื่อนเลือกระดับความเสี่ยงได้, และออกใบสั่งซื้อ/ขาย
ปัดเป็น board lot พร้อมดาวน์โหลด weights.csv / orders.csv
ทุกอย่างคำนวณในเครื่อง ไม่ส่งข้อมูลออกไปไหน
(engine ตัวเดียวกับ `mpt.py` — ทดสอบผลตรงกับ scipy แล้ว)

## การติดตั้ง (เวอร์ชัน CLI)
